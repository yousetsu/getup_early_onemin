...
class _FirstScreenState extends State<FirstScreen> {
  ...
  String strmsg_every_get_up = '毎日〇分ずつ早起き';
  ...
  void initState() {
    ...
    strmsg_every_get_up = '毎日' + int_min_kankaku.toString() + '分ずつ早起き';
    super.initState();
    LoadPref();
...
//FlutterRingtonePlayer.playAlarm();
  ...
  stopTheSound() async {
    ...
    await AndroidAlarmManager.oneShot(Duration(seconds: 0), helloAlarmID, stopSound,
  ...
  Future<void> alramset() async {
    ...
    str_nowtime = _str_date + _hour_now.toString().padLeft(2,'0') + ':' + _minute_now.toString().padLeft(2,'0') + ':' + _second_now.toString().padLeft(2,'0') + '.0';
    ...
    str_getuptime = _str_date + _hour.toString().padLeft(2,'0') + ':' + _minute.toString().padLeft(2,'0') + ':' + _second.toString().padLeft(2,'0') + '.0';
    ...
    if(_diffSecond >= 0) {
      ...
      str_getuptime = _str_date_plusone + _hour.toString().padLeft(2,'0') + ':' + _minute.toString().padLeft(2,'0') + ':' + _second.toString().padLeft(2,'0') + '.0';
      _getupalarmtime = DateTime.parse(str_getuptime);
      _diffSecond = _getupalarmtime.difference(_nowtime).inSeconds;
    }
    ...
    Fluttertoast.showToast(msg: int_hour.toString() + "時間" + int_minute.toString() +"分"+int_second.toString() + "秒後にアラームが鳴ります。");
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: <Widget>[
            ...
            Text('明日起きたい時刻', style: styleB ),

            ElevatedButton(
              child: Text(DateFormat.Hm().format(_getuptime),
            ...
            Visibility(
              ...
              child:TextField(
                ...
                readOnly:true,
                enabled: false,
                style: const TextStyle(fontSize: 40.0, color: Colors.white,decoration: TextDecoration.none),
              ),
            ),
          ],
        ),
  ...
  void LoadPref() async{
    SharedPreferences.getInstance().then(
          ...
          setState(() {
            ...
            //間隔の取得
            if(prefs.getString('kankaku') != null && prefs.getString('kankaku') != "") {
              int_min_kankaku = int.parse(prefs.getString('kankaku')!);
              _controllerTitle.text =
                  '毎日' + (prefs.getString('kankaku') ?? '') + '分ずつ早起き';
            }else{
              _controllerTitle.text = '毎日1分ずつ早起き';
              SharedPreferences.getInstance().then(
                      (SharedPreferences prefs){
            ...
            _goal_bedin_time = _getuptime.subtract(Duration(hours:sleeptime_hour,minutes:sleeptime_min));
  ...
  void saveData(String status) async {
    ...
    String query = 'INSERT INTO rireki(date,getupstatus, goalgetuptime,realgetuptime,goalbedintime,realbedintime,sleeptime) values("$strStartdate","$status","$str_getuptime",null,null,null,null)';
      ...
      body: SingleChildScrollView(
        child: Form(
          ...
          child: Column(
              ...
              children: <Widget>[
                Padding(padding: EdgeInsets.all(20.0),),
                Text('設定', style: styleB,),
                Padding(padding: EdgeInsets.all(20.0),),
                Text('最終目標の起床時刻', style: styleB,),
                TextButton(
                  child: Text(DateFormat.Hm().format(_goalgetuptime),
                      style: const TextStyle(fontSize: 40.0, color: Colors.white)
                ...
                Row(children: <Widget>[
                  Container(
                    ...
                    child: TextFormField(
                      controller: _text_controller_kankaku,  //ここに初期値
                      validator: (value) {
...
class _ThirdScreenState extends State<ThirdScreen> {
  ...
  Widget _listHeader() {
    return Container(
        ...
        child: ListTile(
            title: new Row(
                children: <Widget>[
                  new Expanded(child: new Text("日付", style: new TextStyle(color: Colors.white,fontWeight:FontWeight.bold))),
                  new Expanded(child: new Text("時刻", style: new TextStyle(color: Colors.white,fontWeight:FontWeight.bold))),
                ]
            )
        )
    );
  ...
  void getItems() async {
    ...
    List<Map> result = await database.rawQuery('SELECT id,date,getupstatus,goalgetuptime FROM rireki');
    // List<Map> result = await database.rawQuery('SELECT * FROM rireki');
    for (Map item in result){
      list.add(
          ListTile(
            tileColor: (item['getupstatus'].toString() == cns_getupstatus_s) ? Colors.green : Colors.red,
            leading: (item['getupstatus'].toString() == cns_getupstatus_s) ? Icon(Icons.thumb_up) : Icon(Icons.south),
            subtitle: Text(item['id'].toString()+ ' ' + DateFormat('yyyy/MM/dd').format(DateTime.parse(item['date'])) + ' ' + DateFormat('HH:MM').format(DateTime.parse(item['goalgetuptime']))),
          )
      );
    }
    setState(() {
